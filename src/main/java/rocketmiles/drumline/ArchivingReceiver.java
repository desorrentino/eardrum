...
/**
 * A {@link Receiver} that archives midi data. When a note is played on the
 * device this receiver starts recording to a sequencer. If no notes are played
 * for a set amount of time the recorded sequence will be written to the
 * provided {@link SequenceWriter}. A new recording will start when the next
 * note is played.
 */
public class ArchivingReceiver implements Receiver {
	...
	/**
	 * Closes and released devices. If a recording is in progress it will be stoped
	 * and the recorded data will be archived.
	 */
	@Override
	...
	/**
	 * Prepares and opens a new sequencer. This should be done in advance because if
	 * the sequencer is prepared while processing a midi message it will cause
	 * recording issues on raspberry pi devices.
	 */
